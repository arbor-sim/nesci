...
def main(argv):
    ...
    if stage == 'conan':
        ...
        os.system('conan export-pkg . nesci/%s@RWTH-VR/%s %s -f' % (version, channel, conan_flags))
        os.system('conan test ./test_package nesci/%s@RWTH-VR/%s %s' % (version, channel, conan_flags))
        os.system('conan upload nesci/%s@RWTH-VR/%s --all --force -r=rwth-vr--bintray ' % (version, channel))
